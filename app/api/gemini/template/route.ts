...
export async function POST(req: Request) {
  try {
    const { platform, brand, campaign, hook, tone, cta, cta_link, extra_information } = await req.json();
    ...
    Hook: ${hook}
    Tone: ${tone}
    Call To Action (CTA) text: ${cta}
    CTA Link: ${cta_link}
    Any extra Information: ${extra_information}
    ...
    12. Follow the instructions given strictly, without deviating or hallucinating.
