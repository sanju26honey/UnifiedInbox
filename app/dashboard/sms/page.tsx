...
export default function() {
    ...
    return <>
        <div className="fixed top-5 w-full md:flex mx-auto gap-5  hidden justify-center">
            <div className="flex gap-5">
                <div className="top-5 w-full flex mt-5 justify-start">
                    <Link href="/dashboard/" className="p-3 flex items-center cursor-pointer rounded-full px-5 bg-blue-500 text-white"><HouseSimpleIcon/></Link>
                </div>
                <div className=" top-5 w-full flex mt-5 justify-center">
                    ...
                    <Link href="/dashboard/sms" className="p-3 cursor-pointer rounded-tr-full rounded-br-full px-5 bg-blue-500 text-white">SMS</Link>
                </div>
                <div className="top-5 w-full flex mt-5 justify-end">
                    <button onClick={() => signOut({ callbackUrl: "/login" })} className="p-3 flex items-center cursor-pointer rounded-full px-5 bg-blue-500 text-white"><SignOutIcon/></button>
                </div>   
            </div>     
        </div>
        <div className="grid min-h-screen overflow-hidden gap-5 grid-cols-5">
            ...
            <div className="col-span-5 md:order-2 order-1 md:col-span-3">
                <div className="relative mt-5 h-auto flex justify-between">
                    <div className="top-5 w-full flex md:hidden mt-5 justify-start">
                        <Link href="/dashboard/" className="p-3 flex items-center cursor-pointer rounded-tr-full rounded-br-full px-5 bg-blue-500 text-white"><HouseSimpleIcon/></Link>
                    </div>
                    ...
                    <div className="top-5 w-full flex md:hidden mt-5 justify-end">
                        <button onClick={() => signOut({ callbackUrl: "/login" })} className="p-3 flex items-center cursor-pointer rounded-tl-full rounded-bl-full px-5 bg-blue-500 text-white"><SignOutIcon/></button>
                    </div>
                </div>
                ...
                <div id="quick-start text-center">
                    ...
                    {
                        (messageStatus==1) &&
                        <PropagateLoader className="m-3" speedMultiplier={0.5} color="oklch(70.7% 0.165 254.624)"/>
                    }
                    {
                        (messageStatus==2) &&
            ...
            <div className="order-3 md:col-span-1 col-span-5 border-l-2 px-5 pt-10 border-white/10 overflow-auto md:max-h-screen no-scrollbar">
